  ...
  return <>
    ...
    <div className="deck">
      ...
      <main className="canvas">
        <header className="top"><div><p>APPLY</p><h2>Week 1 · Build momentum deliberately</h2></div><div className="top-actions"><button className="quiet">Log learning gap</button><span className="sync"><i/> {sheetLabel}</span></div></header>
        ...
        <section className="briefing" id="next-action">
          ...
          <div className="brief-actions"><button className="go" onClick={()=>setModal(true)}>Start application package</button><button className="ghost" disabled={!job.url} onClick={()=>job.url&&window.open(job.url,"_blank","noopener,noreferrer")}>Open role posting ↗</button><button className="text-action">Skip for today</button><p>You always submit on the employer&apos;s own form. Nothing here auto-submits.</p></div>
        ...
        <div className="workspace">
          ...
          <section className="paper evidence">
            <div className="section-head"><div><p>SELECTED ROLE</p><h2>{job.title}</h2><span>{job.company}</span></div><button className="mini-go" onClick={()=>setModal(true)}>Start package</button></div>
          ...
          <aside className="side-stack">
            ...
            <section className="paper follow"><p>BEFORE MONDAY</p><h3>Source 5 coffee chats</h3><span>0 of 5 ready</span><button>Open networking</button></section>
          </aside>
        </div>
      </main>
      <nav className="bottom-nav">{["Apply","Today","Pipeline","Network","Review"].map((item,index)=><button className={index===0?"active":""} key={item}><span>{String(index+1).padStart(2,"0")}</span>{item}</button>)}</nav>
    </div>

    {modal&&<div className="scrim"><section className="package" role="dialog" aria-modal="true" aria-labelledby="package-title"><header><div><p>APPLICATION PACKAGE · STEP 2 OF 4</p><h2 id="package-title">{job.company} — {job.title}</h2></div><button onClick={()=>setModal(false)}>Close</button></header><ol><li className="done">1 Evidence</li><li className="current">2 Master blocks</li><li>3 Open & submit</li><li>4 Log result</li></ol><div className="package-body"><h3>Select immutable master-resume blocks</h3><p>Content is locked and copied byte-for-byte. Edit the master resume at its source, never here.</p>{resumeBlocks.map((text,index)=><label className={blocks.includes(index)?"resume-block checked":"resume-block"} key={text}><input type="checkbox" checked={blocks.includes(index)} onChange={()=>setBlocks(current=>current.includes(index)?current.filter(i=>i!==index):[...current,index])}/><span><b>VERBATIM MASTER BLOCK {index+1}</b><small>{text}</small></span></label>)}<div className="manual-note"><b>Manual submission only</b><p>When the package is ready, you will open the employer form in a separate tab and submit it yourself.</p></div></div><footer><span>Local preview draft · Sheets writes disabled</span><div><button className="ghost-paper" onClick={()=>setModal(false)}>Cancel</button><button className="go" disabled={!blocks.length}>Continue to review</button></div></footer></section></div>}
  </>;
}
